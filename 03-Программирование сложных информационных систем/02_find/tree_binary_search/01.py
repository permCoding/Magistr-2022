...
def insert(node, value):
    if node:
        ...
        return Node(value)  # новый узел


def get_tree(lst):
    tree = None
    
    # версия 1: наивная - просто все поочереди добавлять
    # так может получиться однобокое дерево большой глубины - для рекурсии губительно
    # for key in lst: tree = insert(tree, key)
    
    # версия 2: рандомная: в среднем очень хороший результат поиска (почти как бинарный поиск)
    # но слишком большое время генерации дерева для 1 млн и более длины списка
    # arr = lst[:]
    # while len(arr) > 0:
    #     pos = randint(0, len(arr)-1)
    #     elm = arr.pop(pos)  # для больших списков очень долгая операция
    ...
    arr = lst[:]
    shuffle(arr)
    for value in arr: tree = insert(tree, value)
        
    # версия 4: полное бинарное дерево - без перекосов
    # идеальный вариант - попробуйте сделать самостоятельно
    #
    #
    #
...
if __name__ == "__main__":
    ...
    # print_tree(tree)
    ex_e = examine(tree_search, tree, find_number)
    ex_e()  # поиск в бинарном дереве


"""
для 1 млн случайных чисел:
время - позиция, кол-во шагов
a 0.08326912 - [340381, 340382]
b 0.00000000 - [340381, 19]
c 0.00000000 - [340381, 19]
d 0.00000000 - [340381, 3]
e 0.00000000 - [True, 24]

для 10 млн случайных чисел:
a 1.28224373 - [5240825, 5240826]
b 0.00000000 - [5240825, 22]
c 0.00000000 - [5240825, 22]
d 0.00000000 - [5240825, 4]
e 0.00000000 - [True, 31]
